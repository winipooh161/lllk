            ...
            <form action="{{ route('admin.awards.update', $award->id) }}" method="POST">
                ...
                <div class="form-group mb-3">
                    ...
                    <input type="text" class="form-control @error('name') is-invalid @enderror" 
                           id="name" name="name" value="{{ old('name', $award->name) }}" required>
                    @error('name')
                ...
                <div class="form-group mb-3">
                    ...
                    <input type="text" class="form-control @error('category') is-invalid @enderror" 
                           id="category" name="category" value="{{ old('category', $award->category) }}">
                    @error('category')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="form-group mb-3">
                    ...
                    <textarea class="form-control @error('description') is-invalid @enderror" 
                              id="description" name="description" rows="3" required>{{ old('description', $award->description) }}</textarea>
                    @error('description')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group mb-3">
                    ...
                    <div class="input-group">
                        <textarea class="form-control @error('icon') is-invalid @enderror" 
                                 id="icon" name="icon" rows="5" required>{{ old('icon', $award->icon) }}</textarea>
                        @error('icon')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
